...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Janela dos últimos 6 meses para o gráfico de fluxo de caixa,
    // independente do filtro de período/categoria selecionado acima.
    const now = new Date();
      ...
      {activeTab === 'raio_x' ? (
        ...
        <>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {lote.qtd_cabecas} cabeças • Entrada: {formatDate(lote.data_entrada)}
                          </p>
                        </div>
                      </div>
          ...
          <Card className="rounded-2xl shadow-sm border-0 bg-white">
            ...
            <CardContent>
              {transacoes.length === 0 ? (
                <p className="text-center py-6 text-gray-400 text-sm">Nenhuma transação no período</p>
              ) : (
